...
def solution(clothes):
    wear_dict = {}
    for index, [cloth, category] in enumerate(clothes):
        if wear_dict.get(category):
            wear_dict[category].append(cloth)
        else:
            wear_dict[category] = [cloth]

    result = 1
    for key in wear_dict.keys():
        result = result * (wear_dict[key].__len__() + 1)
